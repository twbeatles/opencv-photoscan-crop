...
class LibraryRepositoryRecipeMixin:
    ...
    def get_recipe(self: Any, name: str) -> Optional[dict[str, Any]]:
        with self.store.connect() as conn:
            row = conn.execute(
                """
                SELECT name, description, settings_snapshot, category_rules, origin, created_at, updated_at
    ...
    def delete_recipe(self: Any, name: str) -> None:
        with self.store.connect() as conn:
            conn.execute("DELETE FROM recipes WHERE name = ?", (name,))
            conn.commit()
    def rename_recipe(self: Any, old_name: str, new_name: str) -> bool:
        with self.store.connect() as conn:
            ...
            conn.commit()
            return True
    def set_app_state(self: Any, key: str, value: str) -> None:
        with self.store.connect() as conn:
            conn.execute(
                """
            ...
            conn.commit()
    def get_app_state(self: Any, key: str, default: str = "") -> str:
        with self.store.connect() as conn:
            row = conn.execute(
