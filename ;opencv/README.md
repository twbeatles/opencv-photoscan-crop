...
### 🛡️ 안정성 업데이트 (2026-02)
- **Watch/Batch 파이프라인 일치**: 감시 모드도 배치와 동일한 후처리(얼굴 보정/리사이즈/워터마크/분류 폴더)를 적용
...
> 자동 분류 하위 폴더를 사용할 때도 `skip processed`는 파일명 기반으로 동작하므로, 대량 재처리 전 샘플 검증을 권장합니다.
...
- 🛡️ 얼굴 보정/리사이즈/워터마크/분류 폴더 라우팅을 배치·감시 모드 공통 파이프라인으로 정렬
