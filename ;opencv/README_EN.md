...
- **Watch/Batch Pipeline Parity**: Watch Mode now applies the same post-processing chain as batch mode (face adjustment, resize, watermark, classification folders)
...
> If naming rules/timestamps are enabled, or classification subfolders are used, run a small sample validation before large reruns.
...
- 🛡️ Aligned face adjustment/resize/watermark/classification-folder routing across batch and watch modes
