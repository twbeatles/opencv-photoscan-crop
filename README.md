...
### 자동 사진 감지 & 크롭
- **8단계 지능형 감지 알고리즘**: Canny~LSD + NMS/GrabCut 정제로 다양한 배경에서 높은 검출 성공률
...
- **accurate**: 후보를 모두 수집한 뒤 점수 기반 전역 재랭킹 + 콘텐츠 대비 검증으로 최선 선택
- **장면 프리셋**: 알고리즘 탭에서 스캐너/책상/앨범 등 원클릭 튜닝
- **멀티포토 ROI 재정제**: 분리된 각 사진에 단일 탐지를 한 번 더 적용 (기본 ON)
