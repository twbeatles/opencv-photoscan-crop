...
- **워터마크 추가**: 텍스트/이미지 워터마크 지원 (위치, 투명도, 타일 모드)
- **이미지 리사이즈**: 다양한 모드 (맞춤, 채우기, 비율, 최대 크기)
- **폴더 모니터링 (Watch Mode)**: 폴더에 새 파일 추가 시 자동 처리
- **CLI 모드**: 명령줄에서 배치 처리 가능

### 🎨 UI/UX 개선
- **썸네일 그리드 뷰**: 이미지 목록을 그리드/리스트로 전환
- **전체화면 미리보기**: F11으로 전체화면 프리뷰 (키보드 탐색)
- **플로팅 액션 버튼 (FAB)**: 자주 사용하는 기능 빠른 접근
- **실행 취소/다시 실행**: Ctrl+Z / Ctrl+Y로 히스토리 관리
