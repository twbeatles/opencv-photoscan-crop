...
class ImageDetectionPipelineMixin:
        ...
        *,
        ...
        try:
            ...
            (
                ...
                stage_candidates,
                _edges,
            ) = self._run_detection_stages(
                gray=gray,
                image_area=image_area,
                score_edges_reference=score_edges_reference,
                accurate_full_pass=accurate_full_pass,
                debug_enabled=debug_enabled,
                debug_run_dir=debug_run_dir,
            )

            stage_score_meta = self._stage_score_meta(stage_candidates)
            (
                best_quad,
                best_score,
                best_candidates,
                detection_stage,
            ) = self._finalize_detection_result(
                gray=gray,
            ...
            return self._crop_detected_region(
                orig=orig,
                best_quad=best_quad,
                best_score=best_score,
                best_candidates=best_candidates,
                ...
                debug_run_dir=debug_run_dir,
                original_size=original_size,
                stage_score_meta=stage_score_meta,
            )
        except MemoryError:
    ...
    def _register_stage_candidate_entry(
        self: Any,
        *,
        ...
        accurate_full_pass: bool,
        early_pick: Dict[str, Any],
    ) -> None:
        """Append a stage candidate and optionally set early-exit pick."""
        ...
        if not accurate_full_pass and early_pick.get("quad") is None:
            early_pick["quad"] = entry["quad"]
            early_pick["score"] = entry["score"]
            early_pick["candidates"] = entry["candidates"]
            early_pick["stage"] = stage
    ...
    def _run_detection_stages(
        ...
        *,
        gray: np.ndarray,
        image_area: int,
        score_edges_reference: np.ndarray,
        accurate_full_pass: bool,
        debug_enabled: bool,
        debug_run_dir: Optional[str],
    ) -> Tuple[
        Optional[np.ndarray],
        float,
        List[dict],
        Optional[DetectionStage],
        List[Dict[str, Any]],
        np.ndarray,
    ]:
        """Run stages 1–8 and return early pick + all stage candidates."""
        stage_candidates: List[Dict[str, Any]] = []
        early_pick: Dict[str, Any] = {
            "quad": None,
            "score": 0.0,
        ...
        def _register(
            ...
            self._register_stage_candidate_entry(
                stage=stage,
                quad=quad,
                score=score,
        ...
        _register(stage_1, quad, score, candidates)
        ...
        if (
            early_pick["quad"] is None or accurate_full_pass
        ) and self.algo.detection_mode in ("balanced", "accurate"):
            bgmask = self._create_background_mask(gray)
            quad, score, candidates = self.find_best_contour(
                bgmask, image_area, score_edge_map=score_edges_reference
            )
            _register(DetectionStage.BACKGROUND_MASK, quad, score, candidates)
        ...
        if early_pick["quad"] is None or accurate_full_pass:
            blurred_bilateral = cv2.bilateralFilter(gray, 9, 75, 75)
            adaptive_block_size = int(getattr(self.algo, "adaptive_block_size", 15))
            ...
            _register(DetectionStage.ADAPTIVE_THRESHOLD, quad, score, candidates)
        ...
        if early_pick["quad"] is None or accurate_full_pass:
            ...
            _register(DetectionStage.GRADIENT_SOBEL, quad, score, candidates)
        ...
        should_use_corner = bool(
            self.algo.use_corner_detection or self.algo.detection_mode == "accurate"
        )
        if (early_pick["quad"] is None or accurate_full_pass) and should_use_corner:
            ...
            _register(DetectionStage.CORNER_HARRIS, quad, score, candidates)
        ...
        if (
            early_pick["quad"] is None or accurate_full_pass
        ) and self.algo.detection_mode in (
            "balanced",
            "accurate",
        ):
            ...
            _register(DetectionStage.MORPH_GRADIENT, quad, score, candidates)
            if debug_enabled and debug_run_dir and self.debug.save_detection_stages:
                self._save_debug_image(
        ...
        if (
            early_pick["quad"] is None or accurate_full_pass
        ) and self.algo.detection_mode == "accurate":
        ...
        return (
            early_pick["quad"],
            float(early_pick["score"]),
            list(early_pick["candidates"] or []),
            early_pick["stage"],
            stage_candidates,
            edges,
        )

    def _stage_score_meta(
        self: Any, stage_candidates: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _finalize_detection_result(
        ...
        *,
        ...
        score_edges_reference: np.ndarray,
        accurate_full_pass: bool,
        best_quad: Optional[np.ndarray],
        best_score: float,
        best_candidates: List[dict],
        detection_stage: Optional[DetectionStage],
        stage_candidates: List[Dict[str, Any]],
    ) -> Tuple[Optional[np.ndarray], float, List[dict], Optional[DetectionStage]]:
        """NMS, re-rank, score floor, edge snap, and GrabCut refine."""
        ...
        return best_quad, float(best_score), best_candidates, detection_stage

    def _crop_detected_region(
        self: Any,
        *,
        orig: np.ndarray,
        ...
        """Warp/crop the detected quad and write optional debug artifacts."""
