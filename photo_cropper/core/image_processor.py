...
class ImageProcessor:
    ...
    def detect_edges_multiscale(self, gray: np.ndarray) -> np.ndarray:
        ...
        if self.algo.multi_scale_edge:
            ...
            edges_low = cv2.Canny(
                blurred,
                int(self.algo.canny_min * 0.5),
                int(self.algo.canny_max * 0.5)
            )
            
            # Normal edges
            edges_normal = cv2.Canny(
                blurred,
                self.algo.canny_min,
                self.algo.canny_max
            )
            ...
            edges_high = cv2.Canny(
                blurred,
                int(self.algo.canny_min * 1.5),
                int(self.algo.canny_max * 1.5)
            )
